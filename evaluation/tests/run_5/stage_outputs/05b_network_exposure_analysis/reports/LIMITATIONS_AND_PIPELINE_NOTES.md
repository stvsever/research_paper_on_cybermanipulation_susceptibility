...
- H2 (network propagation) is confirmed at the receiver level: the direction-aware peer-position pull `peer_pull_toward_goal` versus `pn_increment_effectivity` has Pearson `r = 0.240`.

## BN Delta Expansion Note

Stage `02b` canonical profile-opinion assessments report:

- mean `BN - B`: `-0.2537037037037037`
- mean absolute `BN - B`: `51.757407407407406`

Stage `05` expanded scenario rows report:

- mean `network_exposure_delta_score`: `None`
- mean absolute `network_exposure_delta_score`: `None`
