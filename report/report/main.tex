...
\newcommand{\coverlinkoverlays}{%
  \begin{tikzpicture}[remember picture,overlay]
    \node[anchor=south west,inner sep=0pt] at ([xshift=143pt,yshift=281pt]current page.south west)
      {\href{https://github.com/stvsever/research_paper_on_cybermanipulation_susceptibility/tree/main}{\XeTeXLinkBox{\phantom{\rule{27pt}{16pt}}}}};
    \node[anchor=south west,inner sep=0pt] at ([xshift=140pt,yshift=266pt]current page.south west)
      {\href{https://github.com/stvsever/research_paper_on_cybermanipulation_susceptibility/tree/main/evaluation}{\XeTeXLinkBox{\phantom{\rule{28pt}{15pt}}}}};
  \end{tikzpicture}%
}
...
adversarial direction, and a second layer embeds these scenarios in an empirical
social-media exposure network. Across a maximum-entropy sample of 10,000 scenarios scored by
...
supranational-integration positions the most movable, and the political issue domains
...
movement ($\beta = +0.042$, $q = .004$; $\beta = +0.030$, $q = .040$) and conscientiousness
predicted resistance ($\beta = -0.035$, $q = .012$). In the exposure-network layer,
network-conditioned susceptibility was related to, but not reducible to, private
susceptibility, and an attack's network-wide effect increased substantially when comparatively
susceptible profiles occupied higher-reach sender positions (alignment slope $\beta = 2.40$,
$p < .001$). Produced in silico, these estimates are advanced as directional hypotheses for
human validation, and the pipeline is released as a reusable instrument for auditing
cyber-manipulation susceptibility before any citizen is exposed.
\end{abstract}
...
\label{fig:pipeline}
\vspace{2pt}
\includegraphics[width=\linewidth]{../assets/figures/main/MAIN1_pipeline.png}
...
\input{../assets/tables/main/MAIN1_ontological_state_spaces.tex}
...
\input{../assets/tables/main/MAIN2_analytical_decisions.tex}
...
\input{../assets/tables/main/MAIN3_network_state_backbone.tex}
...
\input{../assets/tables/main/MAIN4_network_effect_quantities.tex}
...
\item[\textbf{H1.}] Adversarial operations reliably shift opinions toward the attacker's goal
across the scenario population (central tendency of effectivity $> 0$).
...
\input{../assets/tables/main/MAIN5_profile_moderation.tex}
...
The two layers of the framework answer complementary questions. The individual layer estimates who
is privately susceptible, to which opinions, and through which tactic, while the network layer asks
when those private differences become structurally consequential once opinions circulate through an
empirical exposure graph.

\subsubsection{Individual layer: what is attacked dominates who is attacked}

Across the 8,572 analysed scenarios and 151,448 opinion-leaf measurements, the simulated DISARM
operation moved the private opinion toward the attacker's goal in 89\% of scenarios, with a large
and highly reliable central tendency (median scenario shift $+14.4$ on the 2,001-position scale,
Cohen's $d_z = 1.23$, Wilcoxon $p < 1\mathrm{e}{-300}$), supporting H1. The dominant source of
variation, however, was the opinion targeted rather than the person attacked. The issue domain
explained roughly thirty times more between-scenario variance than the entire 159-feature profile
battery (3.4\% against 0.1\% out of sample), and movability differed sharply across domains
(Kruskal-Wallis $H = 579$, $\varepsilon^2 = 0.067$): technocratic infrastructure and
supranational-integration positions were the most movable, whereas positions anchoring national
identity and institutional integrity were the most resistant, a gradient consistent with motivated
reasoning (H4). The structure of the attack, by contrast, mattered little, since effectivity did
not differ across DISARM Execute tactics or operation-complexity tiers and showed no dose-response;
with no tactic main effect to moderate, the trait-matched-tactic predictions of H3 could not be
supported at the level of DISARM tactic families.

Susceptibility was nonetheless strongly heterogeneous between individuals (leaf-level between-profile
ICC(1) $= 0.83$) yet only weakly aligned with measured traits. The Big Five was the leading
moderator family: openness and neuroticism predicted greater movability and conscientiousness
predicted resistance, all surviving false-discovery-rate correction, whereas the
political-psychology, ideology, and moral-foundations batteries added almost nothing beyond
personality. H2 is therefore supported in direction but qualified in magnitude, and the principal
individual-layer conclusion is that adversarial leverage resides more in the choice of target than
in the profiling of the individual.

\subsubsection{Network layer: susceptibility becomes consequential through sender position}

The network layer first asked whether the profiles that move most in isolation are also those whose
opinions are most amplified by peer exposure. At the profile level ($n = 100$), direct private
susceptibility and post-network amplification were positively but only moderately associated
($r = .28$, 95\% CI $[.08, .47]$), and once profile traits were modelled jointly, direct
susceptibility no longer predicted the network increment (adjusted $\beta = .043$, $p = .736$).
Network-conditioned susceptibility is thus related to, but not reducible to, private susceptibility
(H5): peer exposure preserves part of the individual structure, since neuroticism predicted both
effects, while introducing a distinct social-context component carried more by conscientiousness,
extraversion, and agreeableness.

The decisive network result concerns position rather than person. Using the counterfactual alignment
design over 35 opinion-by-attack conditions on a fixed empirical exposure graph, the network-wide
attack effect increased substantially when comparatively susceptible profiles occupied higher-reach
sender positions. The condition-level alignment slope was positive and large for the primary endpoint,
post-network amplification ($\beta = 2.40$ score points per standard deviation of alignment,
$p < .001$; within-attack permutation $p < .001$), and for the total network attack effect
($\beta = 2.70$, $p = .004$), so that moving from resilient to susceptible high-reach placement
($A_c$ from $-0.9$ to $+0.9$) increased the attack-aligned effect by roughly 4.3 to 4.9 score
points. H6 and H7 are therefore two sides of one mechanism: at the network level, who occupies
influential positions, and not merely who is privately susceptible, governs the reach of an attack.
This is a controlled mechanism estimate rather than a claim that real platforms place susceptible
users at influential positions.
...
language model is not fully equivalent to eliciting actual human response distributions:
...
personality-opinion covariation has not been shown to match the human one for this task.
Behavioural foundation models fine-tuned on large curated corpora of diverse human response
distributions may narrow this gap \citep{zhou2026odyssim}, but the present instrument was not
trained in this way. Such a gap could inflate mean effectivity, consistent with the
89\,percent movement rate, and could distort the trait gradients. The estimates are accordingly
read as simulation-internal directional predictions, and validation in human samples remains
the necessary next step.

A final limitation concerns the empirical substrate of the network layer. Its exposure graph is
derived from a political-discourse subset of the BlueSky platform, whose political user base is
concentrated in particular demographic and ideological communities and whose recorded ties
capture following and reply relations rather than the verified content co-exposure that actually
drives opinion diffusion. The substrate also supports a single network-conditioned update rather
than a multi-round cascade, so the reported amplification is a controlled mechanism estimate
rather than a forecast of long-run propagation. Whether the topology and community structure of
this substrate generalise to mass-reach platforms or to offline contexts is unestablished, so
the external validity of the network-layer conclusions is bounded by how representative BlueSky
is of social interaction in the wider information environment.

\subsection{Future Directions}

Several extensions follow directly from the limitations above and would sharpen what the framework
can claim. The first concerns resolution. Because the maximum-entropy design traded within-cell
depth for breadth, targeted high-iteration runs are its natural complement: sampling many
repetitions of specific profile-by-attack-by-opinion combinations, or restricting a run to narrow
subsets of opinion leaves, would raise statistical power and license claims below the domain-level
resolution of the present study, at the level of individual leaves and configurations.

The validity of the instrument is the precondition for treating any estimate as more than a
simulation-internal artefact, and three complementary checks address the construct-validity and
attenuation concerns raised above. The first is an architecture ablation and component analysis
that would systematically vary the design choices held fixed here, contrasting reasoning-enabled
against reasoning-disabled scoring, several model families and sizes, a gradient of prompt
densities running from the full profile to trait-only subsets, alternative decoding temperatures,
and reversed measurement orders. A variance-components decomposition, for example crossed random
effects over profile, attack, opinion, model, and leaf, would then attribute the observed
variability in effectivity to each source and quantify the share that is instrument-induced rather
than substantive. The second is a meta-analytic calibration that would benchmark the simulated
effect sizes against quantitative human estimates from the empirical persuasion and microtargeting
literature, turning algorithmic fidelity from an assumption into a measured quantity. The third is
an ontology-validation exercise that would re-instantiate the profile, attack, and opinion
ontologies for an entirely different use case, testing whether the ontology-based construction
generalises beyond the present domain.

The construct itself can also be widened. Although the profile ontology already encodes
ethnocultural background and citizenship, these markers were not used as a sampling or conditioning
axis; a cross-cultural and multilingual implementation would systematically vary the cultural
context, a direction made especially pertinent by the geopolitical framing of the threat, since the
same vectors may engage differently across cultural and linguistic mentalities. The ontology could
likewise be extended with an explicit awareness state, namely the degree to which a target
recognises an influence attempt as such, allowing this acute condition to be related to effectivity
and tested for interaction with personality, since the traits that govern susceptibility to an
unrecognised appeal need not be those that govern resistance once it is detected.

A final extension serves the framework's natural higher purpose: coupling each attack vector to its
defensive counterpart, which moves the question from who is movable toward how a population can best
be protected. The same direction-aware machinery that scores an attack can score a countermeasure, so
that interventions such as prebunking, attitudinal inoculation, source-credibility and provenance
cues, or exposure friction could be tested in silico, matched to the profiles and issue domains
where they are most needed, and evaluated at the network level for their capacity to blunt the
amplification that influential sender positions confer. This would convert the instrument from an
audit of adversarial leverage into a design tool for democratic resilience.

\section{Conclusion}

Cyber-manipulation of political opinion is a consequential and growing threat, yet the experiments
that would most directly identify who is susceptible cannot ethically be run on real citizens. The
question of who can be moved, on which issues, and by which technique therefore remains largely
unmeasured. The central contribution of this work is accordingly methodological rather than
substantive. It is an ontology-based, multi-agent simulation framework, released as a command-line
instrument for the structured exploration of influence dynamics, that represents the targeted
person, the adversarial operation, and the opinion as explicit state spaces. Across this joint space
and an empirical exposure network, it measures direction-aware susceptibility before any human is
exposed. Applied at scale, the framework already yields coherent and testable patterns. Attacks move
opinion reliably, what is attacked matters more than who is attacked, susceptibility is heterogeneous
yet only weakly trait-aligned, and private susceptibility becomes consequential once it coincides
with influential network positions. These estimates are best read as directional hypotheses rather
than established human effects, since the present run was deliberately broad and modest in depth,
scored by a single model, and limited to one exposure substrate. The deliverable is therefore less a
finished map of susceptibility than a validated and extensible instrument with which that map can be
drawn, and a defensible methodology for auditing cognitive-security risk in advance of any real-world
exposure.

\clearpage
\section*{AI Acknowledgement}

The following AI-assisted tools were used in the preparation of this study and the associated
codebase. First, Claude Sonnet 4.6 (Anthropic) served as a coding assistant during development of
the ontology-construction pipeline and the data-processing infrastructure in the accompanying
GitHub repository. Second, ChatGPT Image Generation 2.0 (OpenAI) was used to design and create the
visualization of one figure in the main body of the study, namely Figure~\ref{fig:pipeline}, the
pipeline overview. Third, ChatGPT 5.5 (OpenAI) was frequently consulted for refining specific
sentences in a more academic manner throughout the manuscript to improve clarity. All research
ideas, high-level design decisions, and scientific judgments were made exclusively by ourselves.
...
\bibitem[Zhou et~al.(2026)]{zhou2026odyssim}
Zhou, X., Sun, W., Du, W., Liu, J., Sun, H., Ma, Q., Wu, T., Yang, Y., \& Sap, M. (2026).
\textit{OdysSim: Building foundation models for human behavior simulation} [Preprint]. arXiv.
\doiurl{10.48550/arXiv.2606.14199}
...
\input{../assets/tables/sup/SUP1_measurement_reliability.tex}

\clearpage

\input{../assets/tables/sup/SUP2_issue_domain_susceptibility.tex}

\clearpage

\input{../assets/tables/sup/SUP3_curated_moderators.tex}

\clearpage

\input{../assets/tables/sup/SUP4_bigfive_facets.tex}

\clearpage

\input{../assets/tables/sup/SUP5_domain_conditional_moderators.tex}

\clearpage

\input{../assets/tables/sup/SUP6_run_configuration.tex}
