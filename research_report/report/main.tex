...
\item How does exposure to an individual's social network modulate this susceptibility, and
are the profile characteristics that predict the network increment ($\Delta_{\text{network}}$)
the same as those that predict the direct individual effect ($\Delta_{\text{direct}}$)?
\end{enumerate}

\noindent The methodology and results that follow develop the individual layer; the
directional hypotheses associated with RQ1 are stated, with the design, in
...
\begin{figure}[ht]
\floatleft
\caption{Ontology-based Simulation Pipeline for Individual Susceptibility Estimation}
\label{fig:pipeline}
\vspace{2pt}
...
DISARM-red triplet, and one opinion cluster into each scenario; (III)~schema-constrained
agents return a private baseline $B$ and post-attack $P$ for every leaf; (IV)~direction-aware
effectivity $\mathrm{AE}=(P-B)\,d$ is constructed; (V)~the inferential layer estimates attack,
opinion-domain, and profile-moderation effects. The present study runs a 10,000-scenario
sample with the network-exposure layer disabled.}
\end{figure}
...
grounded in the evidence reviewed above. Each maps onto an explicit estimator in the analytic
strategy.
