#!/usr/bin/env bash
# ============================================================
# run_5.sh - Launch test run 5 (single-domain, reduced-profile reference).
#
# Run 5 keeps the run-4 methodology (conformity-calibrated exposure network,
# baseline-polarity guard, sender-reach position metric, receiver-level mechanism
# test) and adds two things aimed at making the INDIVIDUAL layer interpretable:
#
#   1. Reduced profile. The pre-built 10K profiles carry ~520 traits across many
#      overlapping taxonomies, which both dilutes what the agent can condition on
#      and leaves the moderator models under-determined (p >> n). Stage 01 now
#      drops ~40% of the profile (the redundant HEXACO/Eysenck/Hexad personality
#      taxonomies and the low-relevance goals/values/safety/criminal/administrative
#      subtrees) and keeps the research core (demographics, socioeconomics, Big
#      Five, the full political-psychology battery, moral foundations, religion,
#      digital literacy). This filters the already-sampled profiles; it does not
#      re-sample.
#   2. Per-attack resolution. The conditional susceptibility estimator now carries
#      both the pooled task and per-DISARM-Execute-tactic tasks, so a specific
#      attack vector can be selected in the dashboard rather than only the pooled
#      view.
#
# Design (60 scenarios, ONE issue domain, exposure-network layer ON):
#   * scenarios:   60 from the 10,000-row integrated production set (seed 505),
#                  restricted to a single opinion parent cluster:
#                    - Information_Integrity_And_Platforms
#   * each row:    1 reduced full profile + 1 near-unique DISARM Plan/Prepare/
#                  Execute triplet + 1 opinion parent cluster (all directional leaves)
#   * four-state backbone per opinion-cluster leaf: B / BN / P / PN
#   * ~240 LLM calls (60 B + 60 BN + 60 P + 60 PN + a few JSON repairs)
#
# Network-layer scenario governor: the exposure-network layer is capped at 500
# scenarios (--network-scenario-cap). Run 5 is well under the cap so nothing is
...
#
# Usage:
#   bash scripts/tests/run_5.sh                 # single-domain individual + exposure-network
#   bash scripts/tests/run_5.sh --verbose       # + live progress monitor
#   bash scripts/tests/run_5.sh --no-network    # individual layer only
#
# Outputs: evaluation/tests/run_5/ with the interactive dashboard at
# evaluation/tests/run_5/visuals/dashboard_results.html and the comprehensive
# exposure-network report at
# evaluation/tests/run_5/visuals/network_exposure_analysis/reports/.
# ============================================================
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        -h|--help)
            ...
            ;;
        *)
            echo "run_5.sh: unknown argument '$1' (try --verbose, --no-network, --help)" >&2
            exit 2
            ;;
    esac
    shift
...
# The single issue domain this run concentrates into (keeps the exposure network dense).
FOCUS_DOMAINS="Information_Integrity_And_Platforms"
...
echo "============================================================"
echo " RUN 5  |  single-domain reduced-profile individual$( [ "${WITH_NETWORK}" = "1" ] && echo ' + exposure-network' ) layer"
echo " focus domain:     ${FOCUS_DOMAINS} (single)"
echo " scenarios:        60 (seed 505), reduced profile"
echo " verbose:          $( [ "${VERBOSE}" = "1" ] && echo on || echo off )"
echo " network exposure: $( [ "${WITH_NETWORK}" = "1" ] && echo on || echo off )"
echo " output:           evaluation/tests/run_5"
echo "============================================================"

# Pre-flight budget gate. Run 5 makes ~4 LLM calls per scenario with the network
# layer on (B + BN + P + PN); ~240 calls for 60 scenarios.
"${PY}" - <<'EOF'
import os, sys, httpx
...
if spendable < 0.75:
    sys.exit(
        f"Spendable {spendable:.2f} USD is below the 0.75 USD floor for this run. "
        "Top up or raise the key limit at https://openrouter.ai/settings/keys, then re-run."
    )
EOF

mkdir -p "evaluation/tests/run_5/logs"
LOG="evaluation/tests/run_5/logs/run_5_console.log"
...
COMMON_ARGS=(
    --output-root           "evaluation/tests/run_5"
    --run-id                "run_5"
...
for out in ("evaluation/tests/run_5/embeddings", "evaluation/tests/run_5/embeddings_production"):
...
RUN_DIR="evaluation/tests/run_5"
group_into() {
    local group="$1"; shift
...
echo
echo "=== test run 5 complete ==="
echo "Interactive dashboard: evaluation/tests/run_5/visuals/dashboard_results.html"
echo "Network report:        evaluation/tests/run_5/visuals/network_exposure_analysis/reports/run_5_network_exposure_report.html"
