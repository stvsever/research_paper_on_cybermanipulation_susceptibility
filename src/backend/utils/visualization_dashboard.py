...
<div style="padding:32px;background:var(--c-card,#f5f7fa);border-radius:6px;border:1px solid var(--c-line,#dbe3ef);max-width:700px;margin:24px auto;">
  ...
  <p style="color:var(--c-muted,#4a5d7a);line-height:1.7">
    Run <code>embed_ontology(...)</code> from src.backend.utils.semantic_embedding into
...
def _fig_effect_ranking(effect_df: pd.DataFrame, kind: str = "attack", long_df: pd.DataFrame | None = None) -> go.Figure:
    ...
    fig = go.Figure()
    raw_col = "attack_leaf" if kind == "attack" else "opinion_leaf"
...
def _strip_dashes(text: str) -> str:
    """Remove em and en dashes from user-visible markup (style guide: none anywhere)."""
    return (
        str(text)
        .replace(" — ", ", ")
        .replace("—", "-")
...
def generate_research_visuals(
    ...
    for base in [output_root, output_root.parent, output_root.parent.parent, output_root.parent.parent.parent]:
        if embedding_data_path is None and (base / "embeddings" / "embedding_dashboard.json").exists():
            embedding_data_path = base / "embeddings" / "embedding_dashboard.json"
        if production_embedding_path is None and (base / "embeddings_production" / "embedding_dashboard.json").exists():
            production_embedding_path = base / "embeddings_production" / "embedding_dashboard.json"
